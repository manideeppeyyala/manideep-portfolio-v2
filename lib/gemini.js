...
async function chatWithGemini({ apiKey, messages, systemPrompt, temperature = 0.6, maxOutputTokens = 1600 }) {
  ...
  try {
    ...
    for (const model of candidates) {
      ...
      if (geminiRes.status !== 404) break; // 404 = bad model name, try next; anything else won't be fixed by retrying
    }

    if (!geminiRes.ok) {
      console.error("Gemini API error:", lastStatus, lastErrBody);
      return { ok: false, status: 502, code: "UPSTREAM_ERROR", error: "The model provider returned an error.", debugStatus: lastStatus, debugBody: lastErrBody.slice(0, 500) };
    }
